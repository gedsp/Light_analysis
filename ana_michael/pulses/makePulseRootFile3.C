...
void makePulseRootFile3()
{
	...
	TString NA[] = {"AVER_C&D","AVER_Carga","AVER_Descarga"};
	TString folder[] {"2.5MSs/VLED108_2ms_Short_Cable","2.5MSs/VLED108_2ms_Long_Cable"};
	
	
	gStyle->SetOptStat(0);
	
	TCanvas *c108_cable = new TCanvas("c108_cable","c108_cable",1000,1500);
	c108_cable->Divide(2,3);

	for (int i=0; i<3; i++)
	{
		for (int j=0; j<2; j++)
		{
			c108_cable->cd(1+2*i+j);
			h[i+2*i+j] = makePulseHisto(NA[i],folder[j],RC1[2*i+j],RC2[2*i+j]);
			c108_cable->RedrawAxis();
		}
	}
	c108_cable->Print(Form("%s.png",c108_cable->GetName()));
