...
static const int n_names = 28; // PMT plots
...
void plotPMT(TTree* t, int ind, TString name, TString xtitle, double scale=1., double xmin=0, double xmax=0)
{
	...
	else if (ind==(n_names-6)) {x=0.24;y=0.22;}
	...
	{
		TString pmt;
		if (ind==n_names-6) pmt=Form("4096.0*fabs(%s[%d])/pmt_pedrms[%d]",name.Data(),i,i);
		else if (ind==n_names-5) pmt=Form("(pmt_ped_end[%d]-pmt_ped[%d])/pmt_pedrms[%d]",i,i,i);
		else if (ind==n_names-4) pmt=Form("pmt_S1_charge[%d]/pmt_S2_charge[%d]",i,i);
		else if (ind==n_names-3) pmt=Form("pmt_S1_npe[%d]/pmt_S2_npe[%d]",i,i);
		else if (ind==n_names-2) pmt=Form("pmt_S1_amp[%d]/pmt_S2_amp[%d]",i,i);
		else if (ind==n_names-1) pmt=Form("pmt_S1_width[%d]/pmt_S2_gaus_width[%d]",i,i);
		else pmt = Form("%s[%d]",name.Data(),i);
		
		TString cut_zero = Form("%s[%d]!=0",name.Data(),i);
		TString myCuts;
		if (ind<(n_names-6)) myCuts=cuts+" && "+cut_zero;
	...
	if (ind!=2 && ind!=4 && ind!=(n_names-5)) { hs->SetMaximum(15*ymax); if (hs->GetMinimum()==0) hs->SetMinimum(0.11);}
	hs->Draw("hist.nostack");
	...
	*/
	...
	TString xtit[n_names] = {"PMT charge [#muC]", "PMT n.p.e. (x10^{6})", "PMT ped [ADU]", "PMT #sigma_{ped} [ADU]", "PMT ped_end [ADU]", "PMT #sigma_{ped_end} [ADU]",
							 ...
							 "PMT S2_gaus charge [#muC]", "PMT S2_gaus n.p.e. (x10^{6})", "PMT S2_gaus width [#mus]", "PMT S2_gaus amp [V]", "PMT S2_gaus #tau [#mus]",
						 	 ...
						 	 "PMT S1/S2 charge", "PMT S1/S2 npe", "PMT S1/S2 amp", "PMT S1/S2 width"};
	double min[n_names] = {-0.001E-6,-0.01E6,3980.,0.,3980.,0., 
							-0.01E-9,-0.01E3,0.,-0.01,0., 
							-0.001E-6,-0.001E6,-0.001,0.,0., 
							-0.001E-6,-0.001E6,-0.0001,0.,0., 
							-5.1,0.,-10.,
							0,0,0,0};
	double max[n_names] = {0.34E-6,2.E6,4070,10.,4100,10., 
							5.E-9,30.E3,100.,2.05,400., 
							0.4E-6,2.5E6,0.04,0.6,1200., 
							0.5E-6,2.5E6,350,0.12,1200., 
							700.1,5000.,20.,
							0.15,0.18,80,20.0};
	...
	for (int i=0; i<n_names; i++)
	{
		...
		if (i==2 || i==4 || i==(n_names-5)) c1->SetLogy(0);
		else c1->SetLogy(1);
		c1->RedrawAxis();
		c1->Modified();
		c1->Update();
